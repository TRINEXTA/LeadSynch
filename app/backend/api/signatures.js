...
router.post('/contracts/:id/send-for-signature', authenticateToken, async (req, res) => {
  try {
    ...
    const contractNumber = contract.contract_number || contract.reference || contract.id.substring(0, 8);

    await sendEmail({
      to: contract.client_email,
      subject: `Signature de contrat - ${contractNumber}`,
      htmlBody: `
        <h2>Bonjour ${contract.contact_name || 'Client'},</h2>
        <p>Votre contrat est prêt à être signé.</p>
        <p><strong>Contrat N° ${contractNumber}</strong></p>
        <p>Montant: ${contract.total_amount || contract.amount || 0} € TTC</p>
        <br>
  ...
  try {
    ...
    return res.json({
      ...
      contract: {
        ...
        number: contract.contract_number || contract.reference,
        amount: contract.total_amount || contract.amount,
